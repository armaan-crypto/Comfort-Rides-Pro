...
//
//   let createdBookingResponseData = try? JSONDecoder().decode(CreatedBookingResponseData.self, from: jsonData)

import Foundation

// MARK: - CreatedBookingResponseData
struct CreatedBookingResponseData: Codable {
    let booking: Booking
    let errors: [JSONAny]
}
...
struct Booking: Codable {
    ...
    let version: Int
    let status: String
    let locationID, customerID, customerNote: String
    let allDay: Bool
    let appointmentSegments: [AppointmentSegment]
...
// MARK: - CreatorDetails
struct CreatorDetails: Codable {
    let creatorType, teamMemberID: String
